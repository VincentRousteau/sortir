...
class UserController extends AbstractController {
    ...
    #[Route('/user/account', name: 'show_profile', requirements: ['id' => '\d+'])]
    ...
    public function new(AuthenticationUtils $authenticationUtils, ParticipantRepository $participantRepository, Request $request, FileUploader $fileUploader, EntityManagerInterface $em): Response
    {
        ...
        if ($profileForm->isSubmitted() && $profileForm->isValid()) {
            ...
            //Attribution d'un nom par défault pour le fichier
            $fileName = "avatar";
            ...
            $avatar = $serieForm->get('backdropFile')->getData();
            if ($avatar) {
                $backdrop = $fileUploader->upload($avatar, '/avatars');
